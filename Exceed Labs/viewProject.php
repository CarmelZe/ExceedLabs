...
<!-- the php code to view the table -->
		...
		<table class="table" style="position:absolute; left:280px; top:330px; width: 50%">
			<tr>
				<th><h1>Project Name</h1></th>
				<th><h1>Project Date</h1></th>
			</tr>
			...
			<?php
				...
				$sql = "SELECT id,pname,pdate from createproject";
				$result = $conn -> query($sql);

				if($result->num_rows>0){
					while($row = $result-> fetch_assoc()){
						echo "<tr><td>". $row["pname"] ."</td><td>". $row["pdate"] ."</td></tr>";
					}
				echo "</table>";
				}
				else {
	...
	<nav>
		...
		<ul>
			...
			<li><a href="AssignedTasks.html"><i class="fas fa-eye"></i>Assigned Tasks</a></li>
		</ul>
	</nav>
