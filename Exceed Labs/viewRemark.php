...
<body>
	<div class="wrapper">
		...
		<div class="main_content">
			<div class="header"><h1>View Remark</h1></div>
			<div class="info">
				<div> <h4> </h4> </div>
			</div>
		</div>
		...
		<table class="table" style="position:absolute; left:300px; top:330px; width:60%">
			...
			<?php
				...
				$sql = "SELECT ProjectName,Remark,username from remark";
				$result = $conn -> query($sql);

				if($result->num_rows>0){
					while($row = $result-> fetch_assoc()){
						echo "<tr><td>". $row["ProjectName"] ."</td><td>". $row["Remark"] ."</td><td>" . $row["username"] ."</td></tr>";
					}
				echo "</table>";
				}
				else {
